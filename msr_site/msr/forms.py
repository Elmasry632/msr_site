...
class TaskDescForm(forms.ModelForm):
    class Meta:
        ...
        labels = {'task_name': 'Task Name','responsible': 'Responsible','task_desc': 'Task Description','board': 'Task Board'}
        widgets = {'task_desc': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Enter task description'}),'board': forms.Select(attrs={'class': 'form-control'})}